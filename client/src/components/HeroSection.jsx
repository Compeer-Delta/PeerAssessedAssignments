        ...
        <div id='hero' className= 'flex flex-co py-1 w-full bg-slate-800 dark:bg-zinc-800 text-gray-300'>
         ...
         {/* id=hero is used for the footer, when clicked it will zoom back up to the HeroSection*/}
            ...
            prevUrl.includes("/viewsubmissions") == true ?(
                <Link to={prevUrl} state={{modTitle: moduleTitle , nestedPage:"submitwork", modId: moduleId, assignmentTitle: assignmentTitle, assignmentId: assignmentId}} className=" rounded-lg font-Dosis text-slate-900 bg-slate-400 text-center w-[150px] py-4 ml-1  text-m md:text-l font-semibold hover:bg-slate-300"> Back to {prevPageName}</Link>
                ): 
                prevUrl.includes("/upload/") == true ?(
                    <Link to={prevUrl} state={{modId: moduleId , modTitle: moduleTitle}} className=" rounded-lg font-Dosis text-slate-900 bg-slate-400 text-center w-[150px] py-4 ml-1  text-m md:text-l font-semibold hover:bg-slate-300"> Back to {prevPageName}</Link>
                     ):
                prevUrl.includes("/modules/") == true ?(
                <Link to={prevUrl} state={{moduleTitle: (moduleTitle) , nestedPage:"submitwork"}} className=" rounded-lg font-Dosis text-slate-900 bg-slate-400 text-center w-[150px] py-4 ml-1  text-m md:text-l font-semibold hover:bg-slate-300"> Back to {prevPageName}</Link>
                 )
          :(
            <Link to={prevUrl} className=" rounded-lg font-Dosis text-slate-900 dark:text-slate-100 dark:bg-zinc-700 bg-slate-400 text-center w-[150px] py-4 ml-1  text-m md:text-l font-semibold hover:bg-slate-300 dark:hover:bg-indigo-900"> Back to {prevPageName}</Link>
          )
        }
